// -----------------------------
// Main: sectioned Test 7
// Usage:
//   ./test7 SECTION_ID NUM_SECTIONS [BASE_SEED]
// Example:
//   ./test7 17 100 123456789
// -----------------------------
...
int main(int argc, char** argv) {
    ...
    std::cout << "\n=== RANDOM WRONG-ADDRESS ALIASING TEST (SECTIONED) ===\n";
    ...
    std::cout << "TOTAL_TRIALS_IN_SECTION = " << (end_addr - start_addr) << "\n";
    ...
    constexpr uint32_t PROGRESS_INTERVAL = (1u << 20); // print every ~1M addresses

    for (uint64_t true_id_raw = start_addr; true_id_raw < end_addr; ++true_id_raw) {
        const uint64_t true_id = wrap_block_id(static_cast<uint32_t>(true_id_raw));

        uint32_t wrong_id_raw;
        do {
            wrong_id_raw = static_cast<uint32_t>(rng() & ((1ULL << ADDRESS_BITS) - 1));
        } while (wrong_id_raw == true_id_raw);

        const uint64_t wrong_id = wrap_block_id(wrong_id_raw);
        ...
        if ((processed % PROGRESS_INTERVAL) == 0) {
            ...
            std::cout << "[RANDOM TEST PROGRESS] address "
